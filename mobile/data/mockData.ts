// Temporary mock data — matches the shape expected by each screen.
// These values come from the reference design's data.js.
// TODO: replace with real API calls when backend endpoints are confirmed.
...
export const conditions = {
  ...
  peak: 47,
  peakTime: '3 PM',
  severity: 'HIGH' as 'SAFE' | 'CAUTION' | 'HIGH' | 'EXTREME',
};
...
export const routeOrder = ['coolest', 'fastest', 'quietest'] as const;

export const coolSpots = [
  { name: 'Cubbon Park', meta: '4 min · −6° cooler', icon: 'shade', tone: 'green' as const },
  { name: 'BWSSB Fountain', meta: '6 min · mist zone', icon: 'water', tone: 'blue' as const },
  { name: 'Garuda Mall', meta: '8 min · AC refuge', icon: 'ac', tone: 'blue' as const },
];
...
export const heatGrid = Array.from({ length: 70 }, (_, i) => {
  ...
  return palette[(i * 7 + (i % 5)) % palette.length] as 'safe' | 'caution' | 'high' | 'extreme';
});
